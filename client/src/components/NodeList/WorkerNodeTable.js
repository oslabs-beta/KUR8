...
export default function WorkerNodeTable({ nodeData }) {
  ...
  const nodeInfoRows = [];
  const rowHeads = [];
  const { metadata, status, capacity, conditions, nodeInfo } = nodeData;
  const rowNames = ['metadata', 'status', 'capacity', 'conditions', 'nodeInfo'];
  const rowsArray = [
    metadataRows,
    statusRows,
    capacityRows,
  ...
  makeHeads(rowNames);
  makeRows(capacity, capacityRows);
  makeRows(conditions, conditionsRows);
  makeRows(metadata, metadataRows);
  makeRows(nodeInfo, nodeInfoRows);
