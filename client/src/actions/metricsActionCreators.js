...
export const metricsEndpointArray = (query, start, end, step) => [
  ...
  `http://localhost:9090/api/v1/query_range?query=sum by (cpu) (rate(node_cpu_seconds_total{mode="system"}[1m]))&start=${timeFormat(
    4
  )}&end=${new Date().toISOString()}&step=5m`,

  `http://localhost:9090/api/v1/query_range?query=rate(node_network_receive_bytes_total[1m])&start=${new Date(
    new Date().setDate(new Date().getDate() - 1)
  ).toISOString()}&end=${new Date().toISOString()}&step=5m`,

  `http://localhost:9090/api/v1/query?query=100%20-%20(avg%20by%20(instance)%20(rate(node_cpu_seconds_total[1m]))%20*%20100)`,

  `http://localhost:9090/api/v1/query_range?query=sum(container_memory_usage_bytes)%20by%20(node)&start=${new Date(
    new Date().setDate(new Date().getDate() - 1)
  ).toISOString()}&end=${new Date().toISOString()}&step=1m`,

  `http://localhost:9090/api/v1/query_range?query=kubelet_http_requests_total&start=${new Date(
    new Date().setDate(new Date().getDate() - 1)
  ).toISOString()}&end=${new Date().toISOString()}&step=2m`,
  ...
  `http://localhost:9090/api/v1/query_range?query=topk(5,%20rate(container_cpu_usage_seconds_total[5m]))&start=${timeFormat(
    4
  )}&end=${new Date().toISOString()}&step=30s`,
  ...
  `http://localhost:9090/api/v1/query_range?query=sum%20by%20(namespace)%20(kube_pod_status_ready{condition=%22false%22})&start=${new Date(
    new Date().setDate(new Date().getDate() - 1)
  ).toISOString()}&end=${new Date().toISOString()}&step=5m`,
];
